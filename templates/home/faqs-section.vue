...
<template>
  <div class="container mt-[80px] flex flex-col md:mt-[120px]">
    ...
    <div class="relative mt-[60px]">
      <div class="grid grid-cols-1 gap-5 md:grid-cols-2 md:grid-rows-2">
        <div
          v-for="(item, index) in faqsList"
          class="flex flex-col rounded-xl border border-grey-shades-15 p-[30px] md:p-10"
        >
          <p class="text-lg font-medium text-white">{{ item.question }}</p>
          <hr
            class="my-6 block h-[1px] border-0 border-b border-b-grey-shades-15 p-0"
          />
      ...
      <div
        class="bg-section-overlay absolute -bottom-[2px] h-1/5 w-full md:h-1/2"
      ></div>
    </div>

    <div class="mt-0.5 self-center">
      <button-basic>
        Load All FAQ’s
        <template #icon>
          <nuxt-img
